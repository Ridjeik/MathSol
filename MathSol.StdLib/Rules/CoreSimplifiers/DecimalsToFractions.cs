...
[RulePriority(1)]
internal class DecimalsToFractions([FromKeyedServices("construct")] IBuiltinFunctionImplementation construct) : RecursiveRule(construct)
{
    protected override IAstNode ExecuteRecursive(IAstNode node)
    {
        if (node is not NumberNode numberNode || numberNode.IsInteger)
        {
            return node;
        }

        return numberNode.AsFraction();
    }
}
